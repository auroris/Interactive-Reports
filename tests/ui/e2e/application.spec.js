...
import { waitForLoad } from "./support.js";
...
test("loads the stored default document, queries data, paginates from Actions, and changes reports", async ({ page, request }) => {
    const hydrationPosts = [];
    page.on("request", request => {
        if (request.method() === "POST" && queryPath.test(new URL(request.url()).pathname))
            hydrationPosts.push(request);
    });
    const initialLoad = page.waitForResponse(response =>
        new URL(response.url()).pathname === "/api/reports/orders/default");
    await openWorkbench(page);
    const initial = await (await initialLoad).json();
    expect(initial.summary.isDefault).toBe(true);
    expect(initial.result.document.activeTable).toBeTruthy();
    expect(initial.result.rows).toHaveLength(50);
    expect(hydrationPosts).toHaveLength(0);
...
test("exports the current report state as CSV", async ({ page }) => {
    ...
    expect(lines.slice(1).every(line => /^\d+,Acme Corp,/.test(line))).toBe(true);
...
test("a saved report retains its configured tables and switches them without reopening editors", async ({ page, request }) => {
    ...
    try {
        ...
        const saveResponsePromise = page.waitForResponse(response =>
            ...
            && /^\/api\/reports\/[^/]+\/saved$/.test(new URL(response.url()).pathname));
        ...
        await page.reload();
        await expect(page.getByRole("table")).toBeVisible();
        const loadResponse = await waitForLoad(page, () =>
            page.getByRole("combobox", { name: "Saved Report" }).selectOption(String(savedId)));
        expect(modeOf((await loadResponse.json()).result.document)).toBe("pivot");
        ...
        if (savedId) await request.delete(`/api/reports/${savedId}`);
    }
});
...
test("saves and reloads a report, then administers its complete lifecycle", async ({ page, request }) => {
    ...
    try {
        ...
        await expect(page.getByRole("searchbox", { name: "Search" })).toHaveValue("");
        await waitForLoad(page, () => page.locator("interactive-report").evaluate(
            (element, savedReport) => element.setAttribute("saved-report", savedReport), String(savedId)));
        await expect(page.getByRole("searchbox", { name: "Search" })).toHaveValue("Acme Corp");
        await expect(savedSelect).toHaveValue(String(savedId));
        ...
        if (savedId)
            await request.delete(`/api/reports/${savedId}`);
    }
});
...
test("admin uploads a validated report document and downloads its canonical file", async ({ page, request }) => {
    ...
    try {
        ...
        const dialog = page.locator("interactive-report-admin").locator(".ir-dialog");
        await expect(dialog).toBeVisible();
        await dialog.locator("select").selectOption("orders");
        ...
        const uploadResponsePromise = page.waitForResponse(response =>
            response.request().method() === "POST"
            && /^\/api\/reports\/admin\/[^/]+\/documents$/.test(new URL(response.url()).pathname));
...
test("admin sets the database administrator list as a whole", async ({ page, request }) => {
    ...
    const saveList = async dialog => {
        const response = page.waitForResponse(candidate =>
            ...
            && new URL(candidate.url()).pathname === administrators);
        await dialog.getByRole("button", { name: "Save", exact: true }).click();
    ...
    try {
        ...
        let dialog = page.getByRole("dialog", { name: "Administrators", exact: true });
        await expect(dialog).toContainText("workbench-dev");
        await expect(dialog).toContainText("appsettings.json");

        // The Workbench directory answers the picker; typing narrows it to one match.
        const picker = dialog.getByLabel("Add administrator", { exact: true });
        await picker.fill("hopper");
        await expect(dialog.getByRole("option")).toHaveText([/Grace Hopper/]);

        // A typed identity the directory does not know is added locally, then saved in one PUT.
        await picker.fill(identity);
        await expect(dialog).toContainText("No matching users.");
        await dialog.locator(".ir-auth-add").getByRole("button", { name: "Add", exact: true }).click();
        await expect(dialog.getByRole("button", { name: `Remove ${identity}`, exact: true })).toBeVisible();
        await saveList(dialog);
        granted = true;
        await expect(page.getByText("Administrator list saved.", { exact: true })).toBeVisible();

        const listed = await request.get(administrators);
        expect(listed.ok()).toBe(true);
        expect((await listed.json()).database).toContain(identity);

        await page.getByRole("button", { name: "Administrators…", exact: true }).click();
        dialog = page.getByRole("dialog", { name: "Administrators", exact: true });
        // The saved grant is now an identity the server knows, so the picker offers it too;
        // the grant rows are what this checks.
        const grantRows = dialog.locator(".ir-auth-list");
        await expect(grantRows.getByText(identity, { exact: true })).toBeVisible();
        await dialog.getByRole("button", { name: `Remove ${identity}`, exact: true }).click();
        await expect(grantRows.getByText(identity, { exact: true })).toBeHidden();
        await saveList(dialog);
        granted = false;
...
test("a definition edit link and per-column overrides shape the managed report", async ({ page, request }) => {
    ...
    await expect(pencil).toHaveAttribute("target", "_self");
...
test.describe("non-administrator", () => {
    ...
    test("cannot probe a protected report and receives a precise admin denial", async ({ page }) => {
        ...
        await expect(page.getByRole("combobox", { name: "Report", exact: true })).toHaveCount(0);
        const protectedResponse = await page.request.get("/api/reports/regional-summary/default");
