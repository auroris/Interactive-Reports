...
public sealed class SqlSavedReportStore : ISavedReportStore
{
    ...
    private static string CreateTableSql(SavedReportStoreConfig cfg) => cfg.Dialect switch
    {
        ReportDialect.Sqlite => $"""
            ...
            """,
        // TITLE_KEY is already case-folded in code and TITLE_SCOPE carries the ordinal owner
        // identity, so both collate binary: the unique index must agree with the managed ordinal
        // comparison rather than letting the database default (usually case- and often
        // accent-insensitive) merge two distinct owners into one private scope.
        ReportDialect.SqlServer => $"""
            ...
            CREATE TABLE {cfg.TableName} (
                ...
                TITLE_KEY    NVARCHAR(400) COLLATE Latin1_General_100_BIN2 NOT NULL,
                TITLE_SCOPE  NVARCHAR(420) COLLATE Latin1_General_100_BIN2 NOT NULL,
            ...
            """,
        // Invariant: the table name is quoted so DDL and the quoted identifiers SqlKata emits in
        // queries name one object; unquoted, Oracle would fold a mixed-case prefix to upper case.
        // Text columns declare CHAR semantics because the endpoint validates character counts and
        // the default NLS_LENGTH_SEMANTICS is BYTE. The identity column requires the creating
        // schema to hold CREATE SEQUENCE.
    ...
    private static string CreateSourceKeyIndexSql(SavedReportStoreConfig cfg)
    {
        ...
        return cfg.Dialect switch
        {
            ...
            // Provider constraint: Oracle has no partial indexes, and a composite unique index
            // treats two rows whose non-null parts agree as duplicates, so a plain (REPORT_NAME,
